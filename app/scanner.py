...
# Variável global para evitar scans simultâneos
SCAN_ATIVO = False

def scan_network(db: Session):
    global SCAN_ATIVO
    
    # Se já houver um scan rodando, encerra silenciosamente
    if SCAN_ATIVO:
        print("[SCANNER] Ignorado: Já existe uma varredura em andamento.")
        return
    ...
    try:
        ...
        for rede in network_ranges:
            ...
            # Se não achou ninguém, a rede é marcada como inacessível no log
        ...
        for id_rede in ids_configurados:
            if id_rede in ids_alcancaveis:
                # Rede visível: marca quem sumiu como DOWN
                ...
                # Rede invisível: marca quem era 'up' como 'inacessivel'
                afetados = db.query(models.Dispositivo).filter(
        ...
        print(f"[SCANNER] Erro crítico: {e}")
    finally:
        # Libera a trava para o próximo scan
        SCAN_ATIVO = False